...
def human_chr(code: int):
    if code <= 32:
        return "<span class=\"italic\">[" + ["null", "start of heading", "start of text",
                "end of text", "end of transmission", "enquiry",
                "acknowledge", "bell", "backspace", "horizontal tab",
                ...
                "group separator", "record separator", "unit separator", "space"][code].upper() + "]</span>"

    elif code == 127:
        return "<span class=\"italic\">[DEL]</span>"
...
ascii_table = [(str(i), hex(i)[2:], bin(i)[2:], oct(i)[2:], human_chr(i)) for i in range(128)]

tools_list = [
    ("Public IP", "/public-ip", "Shows your current public IP address.<br />Can also be queried via <span class=\"code\">curl</span>, returning the IP as a plain string."),
